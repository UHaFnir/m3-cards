...
export class M3LightCard extends TemplatedCard(LitElement) implements LovelaceCard {
  ...
  private _handleBrightnessInput = (e: CustomEvent<{ value: number }>): void => {
    this._dragValue = e.detail.value;
    this._brightnessThrottle.call(e.detail.value);
  };

  private _handleBrightnessCommit = (e: CustomEvent<{ value: number }>): void => {
    this._brightnessThrottle.flush(e.detail.value);
  };

  private _handleBrightnessDrag = (e: CustomEvent<{ dragging: boolean }>): void => {
    this._dragging = e.detail.dragging;
    if (!e.detail.dragging) this._dragValue = undefined;
  };
    ...
    return html`
      <ha-card style=${`${cssVars} border-radius: ${radius};`}>
        ...
        >
          ...
          ${hasBrightness ? this._renderWaveSlider(displayPct, unavailable, effectiveOn) : nothing}
  ...
  private _renderWaveSlider(pct: number, unavailable: boolean, effectiveOn: boolean) {
    return html`
      <m3-wave-slider
        ...
        .waveStyle=${this._config?.wave_style ?? "wavy"}
        ?disabled=${unavailable}
        label=${this._t("light_brightness_label")}
        @slider-input=${this._handleBrightnessInput}
        @slider-commit=${this._handleBrightnessCommit}
        @slider-drag=${this._handleBrightnessDrag}
      ></m3-wave-slider>
    `;
  }
  ...
  static styles = [
    ...
    css`
      ...
      .brightness-slider {
        --wave-slider-accent: var(--lc-accent);
        --wave-slider-track: var(--lc-track);
        --wave-slider-handle: var(--lc-handle);
      }
